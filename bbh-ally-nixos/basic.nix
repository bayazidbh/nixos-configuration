...

{
  ...
  # enable beesd deduplication
  # (enable only if you use compatible filesystem)
  ...
  jovian = {
    steam = {
      ...
      environment = { # for HDR - not tested deeply
        PROTON_USE_NTSYNC       = "1";
        ENABLE_HDR_WSI          = "1";
        DXVK_HDR                = "1";
  ...
  # Define a user account. Don't forget to set a password with ‘passwd’ before ending installation.
  ...
  environment.systemPackages = with pkgs; [
    ...
    discord vencord vesktop # discord
  ...
  # enable nix-cachyos-kernel binary cache
